...
portfinder.getPorts(2, {}, (portFinderErr, [docsPort, webpackPort]) => {
  ...
  runCmd('webpack-dev-server', `nodemon --watch webpack --watch webpack.app.js --exec webpack-dev-server -- --config webpack.app.js --color --port ${webpackPort} --debug --hot --host ${ip.address()}`);

  runCmd('docs-server', 'nodemon --watch frontend --watch src --exec babel-node frontend/server.js', {
    env: {
      PORT: docsPort,
